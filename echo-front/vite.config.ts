...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      // echo-server source is referenced only via `import type` from
      // `auth-client.ts`. The alias keeps Vite + IDEs happy on resolution
      // even though no echo-server code ever ships in the frontend bundle.
      "@server": path.resolve(__dirname, "../echo-server/src"),
    },
  },
  server: {
    port: 3000,
