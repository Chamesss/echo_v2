...
describe("sortMessages", () => {
  it("orders ascending by seq", () => {
    const out = sortMessages([wire({ id: "b", seq: 3 }), wire({ id: "a", seq: 1 })] as EchoMessage[]);
...
describe("mergeBatch (history paging)", () => {
  it("prepends an older page and keeps order", () => {
    let list = sortMessages([wire({ id: "m3", seq: 3 }), wire({ id: "m4", seq: 4 })] as EchoMessage[]);
  ...
  it("dedupes overlapping pages", () => {
    let list = sortMessages([wire({ id: "m2", seq: 2 }), wire({ id: "m3", seq: 3 })] as EchoMessage[]);
