 ...
 *
 * Objects this app stores in the (shared) bucket all live under the `chat/`
 * namespace — avatars at `chat/users/...`, message attachments at
 * `chat/workspaces/...`. The redirect endpoint will only ever sign keys in this
 * namespace, so it can't be abused to mint URLs for another app's objects in
 * the same bucket.
 */

const OWNED_PREFIX = "chat/";
...
const SAFE_KEY_RE = /^chat\/[A-Za-z0-9/_.-]+$/;
