...
function keyFor(user: string, ext = "png"): string {
  return `chat/workspaces/${ws.workspaceId}/channels/${channel.id}/${user}/${randomUUID()}.${ext}`;
}
...
describe("presign", () => {
  it("scopes the key to workspace/channel/user and leaves images inline", async () => {
    ...
    expect(arg.key).toMatch(
      new RegExp(`^chat/workspaces/${ws.workspaceId}/channels/${channel.id}/${owner.id}/.+\\.png$`),
    );
  ...
  it("rejects a key that doesn't belong to the caller + channel", async () => {
    await expect(
      resolveAttachmentsForSend(ws.workspaceId, channel.id, owner.id, [
        { key: `chat/workspaces/other-ws/channels/x/${owner.id}/abc.png`, filename: "a.png" },
      ]),
    ).rejects.toThrow();
