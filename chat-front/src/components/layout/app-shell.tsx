  ...
  const sidebar = (
    <>
      <div className="flex h-16 shrink-0 flex-col justify-center border-b border-border px-4">
        <div className="truncate text-sm font-semibold text-foreground">{workspace.slug}</div>
        <div className="truncate text-xs capitalize text-muted-foreground">{workspace.role}</div>
      </div>
      ...
      <nav className="flex-1 space-y-3 overflow-y-auto p-2">
        ...
        <div className="space-y-1 border-t border-border pt-2">
          ...
          {session?.user.role === "admin" && (
            <SidebarLink icon={<Shield />} label="Admin" to={paths.workspaceAdmin(workspace.id)} />
          )}
        </div>
      </nav>
      ...
      <div className="shrink-0 border-t border-border p-3">
        <div className="mb-2 px-1 text-xs text-muted-foreground">
          <div className="truncate font-medium text-foreground">{session?.user.name}</div>
  ...
  return (
    <PageTitleProvider>
      ...
      <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
        ...
        <MobileTopBar fallback={workspace.slug} onOpen={() => setMobileOpen(true)} />
...
function MobileTopBar({ fallback, onOpen }: { fallback: string; onOpen: () => void }) {
  ...
  return (
    <div className="flex h-16 shrink-0 items-center gap-2 border-b border-border bg-card px-3 lg:hidden">
      ...
      <div className="truncate text-sm font-semibold text-foreground">{title ?? fallback}</div>
    </div>
  );
}
