...
import {
  logWorkspaceEvent,
  WorkspaceEventName,
} from "../../infrastructure/audit/audit-log.js";
...
export async function getDirectoryController(
  req: Request,
  res: Response,
): Promise<void> {
...
export async function changeRoleController(
  ...
  const member = await members.changeMemberRole(
    req.workspace.id,
    req.params.userId!,
    body.role,
  );
...
export async function leaveWorkspaceController(
  req: Request,
  res: Response,
): Promise<void> {
