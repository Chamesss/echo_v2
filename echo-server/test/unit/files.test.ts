...
describe("isProxyableKey", () => {
  it("accepts owned echo keys", () => {
    expect(isProxyableKey("echo/users/abc/avatar-1.png")).toBe(true);
    expect(isProxyableKey("echo/workspaces/w/channels/c/u/uuid.mp4")).toBe(true);
  });
  ...
  it("rejects other namespaces, traversal, and junk", () => {
    ...
    expect(isProxyableKey("echo/../auction/secret.pdf")).toBe(false);
    expect(isProxyableKey("echo/has space.png")).toBe(false);
...
describe("fileProxyUrl", () => {
  it("builds a stable /api/files pointer with the key url-encoded", () => {
    const url = fileProxyUrl("echo/users/abc/avatar-1.png");
    expect(url).toContain("/api/files?key=");
    expect(url).toContain(encodeURIComponent("echo/users/abc/avatar-1.png"));
  });
});
...
describe("fileRedirectController", () => {
  ...
  it("302s an owned key to a freshly-signed GET", async () => {
    ...
    await fileRedirectController(
      { query: { key: "echo/users/abc/avatar-1.png" } } as unknown as Request,
      res as unknown as Response,
    );
    expect(s3mock.getDownloadUrl).toHaveBeenCalledWith("echo/users/abc/avatar-1.png", expect.any(Number));
